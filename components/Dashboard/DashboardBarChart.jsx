      ...
      <div className="bg-white shadow-md p-2 rounded-md border border-gray-200">
        ...
        {payload.map((item, index) => (
          <p
            ...
            className="text-sm"
            style={{
              color: item.dataKey === "totalSpend" ? "#1e90ff" : "#87cefa", // Custom blue colors
              fontWeight: "600",
            }}
          >
            {item.name === 'Amount' ? "Allocated Budget" : item.name}: {formatCurrencyDashboard(item.value)}
          </p>
        ))}
      </div>
    );
...
export default function ChartDashboard({ budgetList }) {
        ...
        return (
          ...
          >
            ...
            <Bar
              ...
              name="Total Spend"
              stackId="a"
              fill="url(#gradient1)" // Retains gradient
            />
            <Bar
              dataKey="amount"
              name="Amount"
              stackId="a"
              fill="url(#gradient2)" // Retains gradient
            />
          </BarChart>
        );
      case "area":
  ...
  return (
    <Card className="mt-10 border shadow-lg rounded-2xl p-5 bg-gradient-to-br from-white to-blue-100 md:block hidden">
